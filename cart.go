...
func GetItemFromCart() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		usert_id, _ := primitive.ObjectIDFromHex(uid)

		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		defer cancel()

		var filledcart models.User
		err := User_Collec.FindOne(ctx, bson.D{primitive.E{Key: "_id", Value: usert_id}}).Decode(&filledcart)
		if err != nil {
			log.Println(err)
		...
		pointcursor, err := User_Collec.Aggregate(ctx, mongo.Pipeline{filter_match, unwind, grouping})
		if err != nil {
			log.Println(err)
		}
		var listing []bson.M
